...
export class PrismaEventQueue implements EventQueue {
  async enqueue<P = unknown>(events: EventMessage<P>[]): Promise<void> {
    if (!events.length) return

    await prisma.event_queue.createMany({
  ...
  async claim<P = unknown>(topic: string, options?: ClaimOptions): Promise<QueuedEvent<P>[]> {
    ...
    return rows.map((row) => this.mapRow<P>(row))
  }

  async markCompleted(ids: string[]): Promise<void> {
    if (!ids.length) return
    await prisma.event_queue.updateMany({
      where: { id: { in: ids } },
      data: {
        status: 'COMPLETED',
        locked_at: null,
  ...
  private mapRow<P>(row: EventQueueRow): QueuedEvent<P> {
    return {
      id: row.id,
      topic: row.topic,
