...
| `last_error` | String | Any | Scan/Cron | **NEW** - Last error for debugging |
...
- **Before:** Fire-and-forget (errors lost)
- **After:** Await and store errors in `last_error` field
...
- **Free Tier:** 100 trackers, 100 updates/day
- **Starter:** 500 trackers, 500 updates/day
- **Growth:** 2000 trackers, 2000 updates/day
...
1. **Add columns to ShipmentTable:**
   - Shipped Date
   - Estimated Delivery  
   - Actual Delivery
   - Error indicator (⚠️ icon with tooltip)

2. **Add Ship24 sync status:**
   - Show if Ship24 is registered (ship24_tracker_id exists)
   - Show last Ship24 update time
   - Show detailed Ship24 status

3. **Add error visibility:**
   - Show last_error in tooltip or expandable row
   - Highlight shipments with errors
   - Add "Retry Registration" button for failed shipments
...
ALTER TABLE shipments ADD COLUMN IF NOT EXISTS ship24_last_update TIMESTAMP;
```

**Already in schema.prisma** ✓ (need to run migration when DB available)
